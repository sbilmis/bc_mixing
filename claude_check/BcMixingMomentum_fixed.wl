...
(* ProjectSpin1 applies the transverse spin-1 projector
      (g_mn - p_m p_n/p^2)   [no 1/3 — paper convention: Pi_1 = T^{mn} Pi_{mn}].
   The 1/3 that appears in the standard Lorentz decomposition is absorbed into
   the paper's sum-rule formula; the perturbative numerator was derived without
   the 1/3, so the condensate projector must match. *)
