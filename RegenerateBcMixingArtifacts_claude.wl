...
(*
  RegenerateBcMixingArtifacts_claude.wl

  Same as RegenerateBcMixingArtifacts.wl but loads the projector-corrected
  BcMixingMomentum_claude.wl (1/3 removed from ProjectSpin1, see
  ClaudeProjectorFixProcedure.md) and uses order "total" (pert+G2+G3) so the
  dim-6 triple-gluon correction is included in the stability plots.
  Output files get a _claude suffix so the originals are not overwritten.
*)
...
Scan[
  (regenerateLog["  G3 integrand " <> #]; G3BorelIntegrandExpression[#];) &,
  {"AA", "AB", "BB"}
];
...
ExportStabilityDataCSV[regenerateM2Data, "BcMixingThetaVsM2_WangWindow_claude.csv", "M2"];
ExportStabilityDataCSV[regenerateS0Data, "BcMixingThetaVsS0_WangWindow_claude.csv", "s0"];
...
Export["BcMixingThetaVsM2_WangWindow_claude.pdf", regenerateM2Plot];
Export["BcMixingThetaVsS0_WangWindow_claude.pdf", regenerateS0Plot];
...
Print[InputForm[
  <|
    ...
    "M2CSV" -> "BcMixingThetaVsM2_WangWindow_claude.csv",
    "S0CSV" -> "BcMixingThetaVsS0_WangWindow_claude.csv"
  |>
]];
